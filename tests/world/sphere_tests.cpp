...
TEST(Sphere, NormalAtX) {
  auto const s = Sphere{};
  auto const [x, y, z] = NormalAt(s, Point3{1.0f, 0.0f, 0.0f});
...
TEST(Sphere, NormalAtY) {
  auto const s = Sphere{};
  auto const [x, y, z] = NormalAt(s, Point3{0.0f, 1.0f, 0.0f});
...
TEST(Sphere, NormalAtXYZ) {
  ...
  auto const [x, y, z] = NormalAt(s, Point3{root, root, root});
...
TEST(Sphere, NormalAtWithTransform) {
  ...
  auto const [x, y, z] = NormalAt(s, Point3{0.0f, 1.70711f, -0.70711f});
  ASSERT_FLOAT_EQ(x, 0.0f);
...
TEST(Sphere, NormalAtWithScaling) {
  ...
  auto const [x, y, z] = NormalAt(
      s, Point3{0.0f, std::sqrt(2.0f) / 2.0f, -std::sqrt(2.0f) / 2.0f});
